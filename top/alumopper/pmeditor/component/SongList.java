				...
				if(info.exists()){
					...
					try {
						...
						if(new File(f.getPath()+"/"+infos[0]+".wav").exists() && new File(f.getPath()+"/chart.json").exists()){
							...
							sp.info.addInfo(1, InfoBox.ClickOperations.OPEN_FILE,"文件夹缺失文件：",f.getName(),f.getParentFile().getAbsolutePath());
						}
					} catch (FileNotFoundException e) {
						e.printStackTrace();
					}
				}else {
					sp.info.addInfo(1, InfoBox.ClickOperations.OPEN_FILE,"无效的文件夹",f.getName(),f.getParentFile().getAbsolutePath());
				}
			}
		}
