...
public class SelectPanel extends PMPanel{
	...
	public SelectPanel(EditorFrame fr){
		...
		confirm.addActionListener(e -> {
			Editor.chart = songList.songs.get(songList.index)[0];
			info.addInfo(0, new ClickOp(),"正在打开谱面", Editor.chart);
		...
		delete.addActionListener(e -> {
			...
			if((Integer) warn.getValue() == JOptionPane.YES_OPTION){
				...
				for (File f : Objects.requireNonNull(new File("./res/charts/" + selectedChart).listFiles())) {
					if(!f.delete()){
						info.addInfo(2, new ClickOp(),"删除失败","无法删除" + f.getName());
					}
					try {
						Thread.sleep(100);
				...
				if(new File("./res/charts/" + selectedChart).delete()){
					info.addInfo(0,new ClickOp(),"已删除谱面",selectedChart);
				}else {
					info.addInfo(2,new ClickOp(),"删除谱面失败",selectedChart);
				}
				songList.chartScan();
				songList.updateLabel();
				songList.repaint();
	...
	public void createChart(File f, String[] chartInfo){
		...
		if (!qwq.mkdirs()) {
			info.addInfo( 2, new ClickOp(),"创建文件夹失败", qwq.getAbsolutePath());
		}
		...
		songList.chartScan();
		songList.updateLabel();
		info.addInfo(0, new ClickOp(),"成功创建谱面", f.getName());
		try {
