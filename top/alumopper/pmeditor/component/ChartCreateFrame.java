	...
	private final Thread bpmTest = new Thread(){
		...
		public void run(){
			...
			if(v.startsWith("Python exception")){
				parentPanel.info.addInfo(2, new ClickOp() {
					@Override
					public void afterClick(){
						super.afterClick();
						JOptionPane.showMessageDialog(parentPanel,v);
					}
				},"Python错误",v.substring(16));
			}else {
		...
		public void run(){
			while (v == null){
				if(System.currentTimeMillis() - startTime > BPM_MAX_TESTTIME){
					...
					},"bpm测量超时(" + BPM_MAX_TESTTIME + "ms)", "点此手动测量");
				}
			}
		}
	};
	...
	public ChartCreateFrame(SelectPanel jPanel, File f) {
		...
		addchart.addActionListener(e -> {
			...
			if(chartAuthorT.getText().equals("")){
				parentPanel.info.addInfo(1, new ClickOp(),"未填写谱面作者");
			}
			if(musicAuthorT.getText().equals("")){
				parentPanel.info.addInfo(1,new ClickOp(),"未填写曲目作者");
			}
			if(bpmT.getText().equals("")){
				parentPanel.info.addInfo(2,new ClickOp(),"未填写bpm");
				return;
			}else {
				try {
					Double.parseDouble(bpmT.getText());
				}catch (NumberFormatException ne){
					parentPanel.info.addInfo(2,new ClickOp(),"请填写合法的数字作为bpm值");
					return;
				}
			}
			//确认生成谱面
		...
		bpmGet.addActionListener(e -> {
			//测量bpm喵
			parentPanel.info.addInfo(0, new ClickOp(),"开始测量Bpm(最长" + BPM_MAX_TESTTIME + "ms)：",f.getName());
			bpmTest.start();
			bpmTestTimer.start();
